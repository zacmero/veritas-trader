...
def main():
    ...
    account = bot.get_account_summary(currency="ETH")
    ...
    equity_eth = account.get("equity", 0.0)
    equity_usd = equity_eth * spot_price
    ...
    for opt in options_positions:
        ...
        # Prices in ETH, convert to USD
        avg_price_eth = float(opt.get("average_price", 0.0))
        mark_price_eth = float(opt.get("mark_price", 0.0))
        unrealized_eth = float(opt.get("floating_profit_loss", 0.0))
        
        entry_price_usd = avg_price_eth * spot_price
        curr_price_usd = mark_price_eth * spot_price
        unrealized_usd = unrealized_eth * spot_price
    ...
    # 4. Process Hedge (ETH-PERPETUAL) Data
    hedge_table = []
    ...
    if perp_position:
        ...
        realized_eth = float(perp_position.get("realized_profit_loss", 0.0))
        unrealized_eth = float(perp_position.get("floating_profit_loss", 0.0))
        
        realized_usd = realized_eth * spot_price
        unrealized_usd = unrealized_eth * spot_price
    ...
    print(f"{Fore.CYAN}!!! VERITAS TRADER: ETH PERFORMANCE REPORT !!!{Style.RESET_ALL}")

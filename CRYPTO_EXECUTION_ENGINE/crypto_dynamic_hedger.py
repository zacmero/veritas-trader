...
# --- CONFIGURATION ---
SLEEP_INTERVAL = 5
DELTA_BAND = 0.005  # Rebalance if Net Delta strays beyond +/- 0.005
    ...
    while True:
        try:
                ...
                try:
                    ...
                    if "delta" in greeks and "mark_iv" in opt:
                        ...
                        iv = prime_math.get_implied_volatility(
                            market_price=opt_mark_btc * spot_price,
            ...
            if abs(true_net_delta) >= DELTA_BAND:
                ...
                # Round to nearest contract size ($10 for BTC)
                contract_size = 10.0
                ...
                if trade_amount_usd != 0:
                    ...
                    # Deribit BTC tick size is $0.50
                    limit_price = round(spot_price * 2) / 2.0

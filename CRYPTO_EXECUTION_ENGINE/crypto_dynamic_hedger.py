        ...
        try:
            ...
            for opt in options_positions:
                try:
                    ...
                    if is_expiration_danger(occ_data["expiry"]):
                        ...
                        # 2. Liquidate Hedge
                        current_perp = int(perp_positions.get("BTC-PERPETUAL", 0.0))
                        if current_perp > 0:
                            bot.place_market_sell("BTC-PERPETUAL", int(abs(current_perp)))
                        elif current_perp < 0:
                            bot.place_market_buy("BTC-PERPETUAL", int(abs(current_perp)))
                    ...
                    current_hedge_usd = float(perp_positions.get("BTC-PERPETUAL", 0.0))
                    hedge_delta_coin = current_hedge_usd / spot_price
                    true_net_delta = total_position_delta_btc + hedge_delta_coin

                    print(f"OPT: {symbol} {expiry_str} | S: ${spot_price:.2f} | IV: {iv*100:.1f}%")
                    print(f"DELTAS -> Option: {total_position_delta_btc:.3f} | Hedge: {hedge_delta_coin:.3f} | TRUE NET: {true_net_delta:.3f}")
            ...
            for underlying, target_usd in target_perp_hedges.items():
                ...
                if abs(usd_to_trade) >= HEDGE_THRESHOLD_USD:
                    ...
                    # Note: Use total_position_delta_btc here
                    projected_net_delta = total_position_delta_btc + projected_hedge_delta

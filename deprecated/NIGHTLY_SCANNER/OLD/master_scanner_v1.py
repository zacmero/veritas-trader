...
# --- IMPORTS ---
# Make sure parser12.py is in the same directory as this script
...
def analyze_news_context(ticker):
    logging.info(f"--- [News System] Fetching news context for {ticker}... ---")
    try:
...
def run_audit_mode(targets, archetypes_db):
    ...
    for ticker in targets:
        ...
        for m_value in M_VALUES_TO_SCAN:
            ...
            if len(ts) < m_value * 2: 
                logging.warning(f"  Not enough data for m={m_value}. Skipping.")
                continue
            ...
            for valley_idx in valleys:
                ...
                log_prefix = "  -> [LIVE]" if is_recent else "  -> [HIST]"
                logging.info(f"{log_prefix} Analyzing valley at {valley_date.strftime('%Y-%m-%d')}")
                
                if valley_idx < DNA_WINDOW: continue
                past_30_day_pattern = ts[valley_idx - DNA_WINDOW + 1 : valley_idx + 1]
                
                # We call the filters for EVERY valley to show the work
                entry_signal = run_entry_filters(past_30_day_pattern, archetypes_db)
                
                if entry_signal: # This means it passed F2 and F3
                    logging.info(f"     [F2/F3 PASSED] Match: {entry_signal['match_name']} ({entry_signal['match_type']})")
                    if is_recent:
                        live_alerts_found += 1
                    ...
                    # This block was missing, now it provides the reason for failure
                    logging.info(f"     [REJECTED] Filters failed.")

            logging.info(f"--- Result for m={m_value}: {live_alerts_found} live alerts generated. ---")

def main():
    parser = argparse.ArgumentParser(description="Master Scanner v1.0")

...
SELL_LIBRARY_PATH = os.path.join(BASE_PATH, "..", "SELL_ARCHETYPE_LIBRARY")
...
SELL_DNA_THRESHOLD = 6
SELL_DNA_WINDOW = 30 # The length of the sell archetype pattern itself
SELL_DNA_SEARCH_WINDOW = 60 # The window of recent data to search within

# --- NEW VOLATILITY FILTER PARAMETERS ---
...
# --- NEW: SELL FILTER 4 ---
def run_sell_filter(current_ts_window, sell_archetypes_db):
    if not sell_archetypes_db:
        return None

    normalized_top = z_normalize(current_ts_window)
    best_match_name, best_match_distance = None, np.inf

    for name, archetype in sell_archetypes_db.items():
        try:
            if len(normalized_top) > len(archetype): continue
            distance_profile = stumpy.mass(archetype, normalized_top)
            min_distance = np.min(distance_profile)
    ...
    if best_match_distance <= SELL_DNA_THRESHOLD:
        logging.info(Fore.YELLOW + f"  [Filter 4] SELL DNA MATCH. Pattern resembles '{best_match_name}' (Distance: {best_match_distance:.4f}).")
        return {'match_name': best_match_name, 'distance': best_match_distance}
...
def main(args):
    ...
    sell_archetypes_db = load_archetypes(SELL_LIBRARY_PATH)
    ...
    for i in range(args.m, len(full_data)):
        ...
        if in_trade:
            ...
            # --- EXIT 1: PRIMARY (Filter 4 - Intelligent Peak Confirmation) ---
            if i >= DNA_WINDOW:
                past_30_day_pattern = ts[i - DNA_WINDOW + 1 : i + 1]
                
                if sell_pattern_confirmed:
                    # If pattern is already confirmed, we sell after a significant drop from the peak.
                    peak_price = np.max(past_30_day_pattern)
                    percentage_diff = ((current_price - peak_price) / peak_price) * 100.0
                    if percentage_diff < -PEAK_SELL_OFF_PERCENTAGE:
                        exit_reason = "Peak Sell-Off"
                else:
                    # Otherwise, we check if we should confirm the pattern.
                    sell_signal = run_sell_filter(past_30_day_pattern, sell_archetypes_db)
                    if sell_signal:
                        sell_pattern_confirmed = True
                        logging.info(Fore.YELLOW + f"  [!] Sell Pattern '{sell_signal['match_name']}' confirmed on {current_date.strftime('%Y-%m-%d')}. Monitoring for sell-off.")

            # --- EXIT 2: EMERGENCY (ATR Trailing Stop) ---
            if not exit_reason: # Only check ATR if pattern match hasn't fired
            ...
            if exit_reason:
                ...
                logging.info(Fore.YELLOW + f"\n!!! SELL SIGNAL on {current_date.strftime('%Y-%m-%d')} at ${current_price:.2f}. Reason: {exit_reason}. Profit: {profit:.2f}% !!!\n")

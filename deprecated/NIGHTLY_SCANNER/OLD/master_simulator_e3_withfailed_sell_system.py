...
# --- NEW, CORRECT SELL PARAMETERS ---
PEAK_SELL_OFF_PERCENTAGE = 20.0 # Trigger "Sell Watch" after a 20% drop from the peak
...
def run_sell_filter(current_ts_window, sell_archetypes_db):
    ...
    best_match_name, best_match_distance = None, np.inf
    normalized_pattern = z_normalize(current_ts_window)

    for name, archetype in sell_archetypes_db.items():
        try:
            if len(normalized_pattern) > len(archetype): continue
            distance_profile = stumpy.mass(archetype, normalized_pattern)
            min_distance = np.min(distance_profile)
    ...
    if best_match_distance <= SELL_DNA_THRESHOLD:
        logging.info(Fore.YELLOW + f"  [Filter 4] CRASH DNA MATCH! Pattern resembles '{best_match_name}' (Distance: {best_match_distance:.4f}).")
...
def main(args):
    ...
    # --- NEW: Variables for the CORRECT exit logic ---
    peak_price_in_trade = 0.0
    ...
    for i in range(args.m, len(full_data)):
        ...
        if in_trade:
            ...
            # --- STAGE 1: PEAK SELL-OFF (Activates the Watch) ---
            if not sell_watch_active:
                drawdown = ((peak_price_in_trade - current_price) / peak_price_in_trade) * 100.0 if peak_price_in_trade > 0 else 0
                if drawdown >= PEAK_SELL_OFF_PERCENTAGE:
                    sell_watch_active = True
                    logging.info(Fore.YELLOW + f"  [!] SELL WATCH ACTIVATED on {current_date.strftime('%Y-%m-%d')}. Price dropped >{PEAK_SELL_OFF_PERCENTAGE}% from peak. Now scanning for crash DNA.")

            # --- STAGE 2: CRASH DNA CONFIRMATION (The Sell Trigger) ---
            if sell_watch_active:
                if i >= DNA_WINDOW:
                    current_30_day_pattern = ts[i - DNA_WINDOW + 1 : i + 1]
                    sell_signal = run_sell_filter(current_30_day_pattern, sell_archetypes_db)
                    if sell_signal:
                        exit_reason = f"Crash Pattern Match ({sell_signal['match_name']})"

            # --- EXIT 3: EMERGENCY (ATR Trailing Stop) ---
            if not exit_reason:
